...
export const env = createEnv({
  server: {
    ...
    /**
     * DEVELOPMENT-ONLY admin bootstrap credentials.
     *
     * Consumed exclusively by `prisma/seed.ts` to create/update a local
     * development admin account. They are optional (never required at
     * runtime) and MUST be left empty in production - no demo credentials
     * are ever created automatically outside local development.
     */
    DEV_ADMIN_EMAIL: z.string().email().optional(),
    DEV_ADMIN_PASSWORD: z.string().min(8).optional(),
    DEV_ADMIN_NAME: z.string().default("Development Admin"),
