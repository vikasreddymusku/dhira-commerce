...
// Loads .env for local dev whether the seed runs via `npm run db:seed`,
// `prisma db seed`, or directly with tsx.
 ...
 * variables. Never hardcodes credentials and never runs in production.
 *
 * Password is hashed with bcrypt (cost 12). The account is upserted so
 * re-running the seed keeps local credentials in sync with `.env`.
 */
async function seedDevelopmentAdmin() {
  ...
  const name = process.env.DEV_ADMIN_NAME?.trim() || "Development Admin";

  if (!email || !password) {
    console.log(
      "Skipping development admin seed (DEV_ADMIN_EMAIL / DEV_ADMIN_PASSWORD not set).",
    );
    return;
  }
  ...
  console.log(`Seeded development admin account for ${email}.`);
}
